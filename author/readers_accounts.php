...
<section class="accounts">
   ...
   <?php
      $select_account = $conn->prepare("SELECT * FROM `readers`");
      $select_account->execute();
      if($select_account->rowCount() > 0){
         while($fetch_accounts = $select_account->fetch(PDO::FETCH_ASSOC)){ 
            $reader_id = $fetch_accounts['id']; 
            $count_user_comments = $conn->prepare("SELECT * FROM `comments` WHERE reader_id = ?");
            $count_user_comments->execute([$reader_id]);
            $total_user_comments = $count_user_comments->rowCount();
            $count_user_likes = $conn->prepare("SELECT * FROM `likes` WHERE reader_id = ?");
            $count_user_likes->execute([$reader_id]);
            $total_user_likes = $count_user_likes->rowCount();
   ?>
   <div class="box">
      <p> users id : <span><?= $reader_id; ?></span> </p>
